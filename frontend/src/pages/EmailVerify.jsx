...
const EmailVerify = () => {
    ...
    const { backenUrl, isLogin, userData, getUserData } = useContext(AppContext)
    ...
    const onSubmitHandler = async (e) => {
        try {
            ...
            const { data } = await axios.post(backenUrl + '/api/auth/verify-account', { otp })
    ...
    return (
        <div className='flex items-center justify-center min-h-screen px-6 sm:px-0 bg-gradient-to-br from-blue-200 to-purple-400'>
            ...
            <form onSubmit={onSubmitHandler} className='bg-slate-900 p-8 rounded-lg shadow-lg w-96 text-sm'>
                ...
                <button className='w-full py-3 bg-gradient-to-r from-indigo-500 to-indigo-900 text-white rounded-full'>Verifyy email</button>
            </form>
        </div>
    )
}
